...
const dynamicText = document.querySelector('.conteiner p');

function doDinamic(arr, dynamicText) {
    const index = Math.floor(Math.random() * (arr.length - 1));
    dynamicText.innerHTML = arr[index]
}
setInterval(doDinamic, 3000, arr, dynamicText)